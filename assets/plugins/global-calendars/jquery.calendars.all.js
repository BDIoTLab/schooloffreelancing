...
(function($) { // Hide scope, no $ conflict
	...
	$.extend(Calendars.prototype, {
		...
		timeInTS: function(Time) {
			var count = 0;
			var split1 = Time.split(" ");
			var split2 = split1[0].split(":");

			if(split1[1].toLowerCase() == "pm" && split2[0] != "12"){
				count += 12*60*60;
			}

			if(split1[1].toLowerCase() == "am" && split2[0] == "12"){
			}else{
				count += split2[0]*60*60;
			}
			count += split2[1]*60;
				...
				if (inst.inline || plugin.curInst === inst) {
					if ($.isFunction(inst.options.onChangeMonthYear) && (!inst.prevDate ||
						...
						inst.options.onChangeMonthYear.apply(elem[0],
							[inst.drawDate.year(), inst.drawDate.month(),inst]);
					}
				}
				if (inst.inline) {
		...
		_generateMonth: function(elem, inst, year, month, calendar, renderer, first) {
			...
			for (var week = 0; week < numWeeks; week++) {
				...
				for (var day = 0; day < calendar.daysInWeek(); day++) {
					...
					var addedJdd = drawDate.formatDate('dd-mm-yyyy');
					days += this._prepare(renderer.day, inst).replace(/\{day\}/g,
						(selectable ? '<a href="javascript:void(0)"' : '<span') +
						' class="jd' + jd + " jdd" + addedJdd + ' ' + (dateInfo.dateClass || '') +
