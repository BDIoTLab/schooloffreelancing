...
class LoginController extends Controller {
	...
	public function searchStudents($student){
		$students = \User::where('role','student')->where(\DB::raw('CONCAT(firstName, IF(secondName IS NULL || secondName == "", "", secondName), IF(thirdName IS NULL || thirdName == "", "", thirdName), familyName)'),'like','%'.$student.'%')->orWhere('passport','like','%'.$student.'%')->get();
		$retArray = array();
