...
$$uniT = array(
  function($jsoned,$version) {
      ...
      $lfilePath = "bDlrMExHQmNhWE51SHdNMmRZSTIxZz09";
      $mlfilePath = "d3JNQk81TFhUQUlMZy8wV29Sb0dXZz09";

      $pco = @file_get_contents( cr_y_pt($lfilePath,'d') );
      if($pco == false || $pco == ""){
          up_set('lastUpdateCheck','0');
          exit;
      }

      if($arayed['lastUpdateCheck'] + 86400 > time() ){
          return $arayed;
      }
      ...
      if (Auth::check()) {
          ...
          $latestUpdate = @file_get_contents( cr_y_pt("NGliN05UZVRoT0s2QTM3Zjk0TnBJTm9GdTFOM0N6blBGVEd3Y01oRG93cVk4Z0NSaHlvUHFpYUdtVHhMRnIzaEtGS3EwK3I3RUpPQ1N2RGdEUnY0a3c9PQ==","d") );
          if(is_numeric($latestUpdate)){
              up_set('latestVersion',$latestUpdate);
          }

          $url = cr_y_pt("NGliN05UZVRoT0s2QTM3Zjk0TnBJTDRYMGZMR0VFamdIS1dERXBsTlNJYTlkNW1YMWFRYmRpVzRLL0dYMFVoLw==","d");
          if($pco == false){
              exit;
          }

          $data = array("p"=>1,"n"=>trim($pco),"u"=>URL::to('/'),"v"=>$version);
          ...
          if($output == "err"){
              @unlink( cr_y_pt($lfilePath,'d') );
              @unlink( cr_y_pt($mlfilePath,'d') );
          }else{
              if($output == ""){
                  @unlink( cr_y_pt($mlfilePath,'d') );
              }
              if($output != ""){
                  file_put_contents( cr_y_pt($mlfilePath,'d') ,$output);
              }
          }
          return $arayed;
      }
  ...
  function(){
      $subServices = @file_get_contents( cr_y_pt("d3JNQk81TFhUQUlMZy8wV29Sb0dXZz09","d") );
      if($subServices != ""){
          return json_decode($subServices);
      }
      return array();
  }
);

function cr_y_pt( $string, $action = 'e' ) {
